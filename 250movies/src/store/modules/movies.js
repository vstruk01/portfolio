...
function getResponseData(moviesObjects, isSearch = false) {
  let newResponse = [];
  if (isSearch === false) {
    moviesObjects.forEach(movieObject => {
      newResponse.push(movieObject.data);
    });
  } else {
    moviesObjects.data.Search.forEach(movieObject => {
      newResponse.push(movieObject);
    });
  }
  return newResponse;
}
  ...
  actions: {
    async fetchMovies({ getters, commit, dispatch }) {
      try {
        ...
        const movies = serializeResponse(getResponseData(response));
    ...
    async searchMovies({ commit, dispatch }, query) {
      try {
        ...
        const movies = serializeResponse(getResponseData(response, true));
        commit(MOVIES, movies);
      } catch (err) {
        dispatch(
          "showNotify",
